...
class UpsModel (AbstractModel):
    ...
    def __init__(self, canvas, m1, m2, nbins=175, user_labels=None):
        ...
        self.m1s = ROOT.RooRealVar("m1s", "mean(%s)" % name, m_y1s,
                                   m_y1s - 0.2 * s_y1s, m_y1s + 0.2 * s_y1s)

        self.sigma = ROOT.RooRealVar("sigma", "sigma(%s)" % name, s_y1s,
                                     0.5 * s_y1s,         2.0 * s_y1s)

        self.n = ROOT.RooRealVar("n", "n", 4, 1, 5)

        self.alphaL = ROOT.RooRealVar("alphaL", "alphaL", 1.28,
                                      1.0, 3.5)
        ...
        self.n1s = ROOT.RooRealVar("N1S", "Signal(Y1S)",  100,  0,  1.e+10)
        ...
        self.n2s = ROOT.RooRealVar("N2S", "Signal(Y2S)",  100,  0,  1.e+10)
        self.n3s = ROOT.RooRealVar("N3S", "Signal(Y3S)",  100,  0,  1.e+10)
        self.b = ROOT.RooRealVar("B", "Background",   10,  0,  1.e+10)
