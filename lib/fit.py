...
from selector import UniversalSelector
...
class Fit(object):
    ...
    def histogram(self):
        ...
        self.tuples.Draw('%s >> %s' %
                         ...
                         utils.cut_expr(self.cut),
                         "Setw2")

    def source(self):
        print "CUT expr %s" % utils.cut_expr(self.cut)
        if self.is_unbinned:
            if self.has_splot:
                ...
                columns = [self.field]
            selector = UniversalSelector(self.cut, columns=columns)
