...
BINNINGS = [[(6, 8), (8, 10), (10, 12), (12, 14)],
           [(14, 18), (18, 22), (22, 30), (18, 30)]]
# BINNINGS = [[(6, None)]]
# ============================================================================
...
for BINNING in BINNINGS:
    ...
    for bin in BINNING:
        for year in ("2011", "2012"):
            ...
            mu += prefix + \
                "%s" % utils.latex_ve_pair(VE(str(fit["m1s"])) * 1000)
            dm2 += prefix + \
                "%s" % utils.latex_ve_pair(VE(str(fit["dm2s"])) * 1000)
            dm3 += prefix + \
                "%s" % utils.latex_ve_pair(VE(str(fit["dm3s"])) * 1000)
            sigma += prefix + \
                "%s" % utils.latex_ve_pair(VE(str(fit["sigma"])) * 1000)
    ...
    print ("\subtable[$%d < p_T^{\Upsilon} < %d \gevcc$]{" % (BINNING[0][0], BINNING[-1][1]) if len(BINNINGS) > 1 else "")
